...
private fun Flow(
    ...
    Layout(content, modifier) { measurables, outerConstraints ->
        ...
        fun canAddToCurrentSequence(placeable: Placeable) =
            currentSequence.isEmpty() || currentMainAxisSize + mainAxisSpacing.roundToPx() + placeable.mainAxisSize() <= constraints.mainAxisMax
        ...
        layout(layoutWidth, layoutHeight) {
            sequences.forEachIndexed { i, placeables ->
                val childrenMainAxisSizes = IntArray(placeables.size) { j ->
                    placeables[j].mainAxisSize() + if (j < placeables.lastIndex) mainAxisSpacing.roundToPx() else 0
                }
...
private fun FlowMainAxisAlignment.toHorizontalArrangement() =
    when (this) {
        ...
        MainAxisAlignment.Start -> Arrangement.Start
        ...
        MainAxisAlignment.SpaceAround -> Arrangement.SpaceAround
    }

private fun FlowMainAxisAlignment.toVerticalArrangement() =
    when (this) {
        MainAxisAlignment.Center -> Arrangement.Center
        MainAxisAlignment.Start -> Arrangement.Top
        MainAxisAlignment.End -> Arrangement.Bottom
        MainAxisAlignment.SpaceEvenly -> Arrangement.SpaceEvenly
        MainAxisAlignment.SpaceBetween -> Arrangement.SpaceBetween
        MainAxisAlignment.SpaceAround -> Arrangement.SpaceAround
    }
