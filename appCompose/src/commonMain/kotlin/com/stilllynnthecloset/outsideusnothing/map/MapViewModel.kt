...
internal class MapViewModel constructor(
    ...
    suspend fun generateDegrees(node: HexGridNode, degrees: Int) {
        if (degrees <= 0) {
            return
        }
        pickRandomPortName()
    ...
    suspend fun generatePort(node: HexGridNode): HexGridNode {
...
private class Graph constructor(
    ...
    fun dijkstra(startCoordinate: HexGridCoordinate): Graph {
        ...
        val q = PriorityQueue(graph.values.toMutableList())
    ...
    private class PriorityQueue<T : Comparable<T>> constructor(val values: MutableList<T>) {
        fun isNotEmpty(): Boolean = values.isNotEmpty()

        fun remove(): T = values.removeAt(0)

        fun remove(element: T): Boolean = values.remove(element)

        fun add(element: T) = values.add(element).also { values.sort() }
    }
